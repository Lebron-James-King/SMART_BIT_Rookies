...
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "2"   # os.environ['环境变量名称']='环境变量值' #其中key和value均为string类型
import argparse
import warnings
import sys
...
def main(args: argparse.Namespace):
    ...
    # Setup logdir.
    if not args.logdir:   # 如果我们没有指定保存地址，就执行下面代码，默认放到/train/logs/文件夹下，以程序开始执行时间作为名字
        time = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")     # 从这里就可以看到我们训练模型保存的名字了
    ...
    for scen in config["scenarios"]:          # 遍历训练多种场景
...
def run(
    ...
    if config["mode"] == "train":
        ...
        model = getattr(sb3lib, config["alg"])(      # getattr() 函数用于返回一个对象属性值
